 ...
 */
class Shaw_Email_Transport_File 
	extends Zend_Mail_Transport_Abstract
{
    ...
    protected function _sendMail()
    {
        ...
        $email = '<html><body>' . PHP_EOL;
        $email.= nl2br($this->header);
        $email.= '</body></html>' . PHP_EOL;
		$email.= '<hr />' . PHP_EOL;
        //$email.= nl2br($this->_mail->getBodyText()->getRawContent());
        $email.= '<hr />' . PHP_EOL;
        $email.= $this->_mail->getBodyHtml()->getRawContent();
    ...
    {
        return 'email_' . $this->recipients . '_' . $_SERVER['REQUEST_TIME'] . '_' . mt_rand() . '.htm';
    }
}
