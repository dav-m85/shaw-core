...
class Shaw_Proxy_ICMP {
    ...
    */
    function callback($type, $data) {
        ...
        //if(! is_string($cbk && is_callable($cbk))){
        call_user_func($cbk, $type, $data);
        //}
    ...
    */
    function ping($hostname, $maxpackets = 3) {
        ...
        $data = "08 00 00 00 08 97 00 00 " . "65 00 00 39 8e d8 3c a1 " . "e1 05 " . "00 08 09 0a 0b 0c 0d 0e 0f 10 11 12 13 14 " . //data
        ...
        for ($i = 0; $i < $this->maxpackets; $i++) {
            ...
            if ($aux > 1) {
                ...
                if ($num > 0) {
                    ...
                    //view_packet($val3);
                    $src = array_slice($data_recv, 11, 4);
                    array_walk($src, "hex2dec");
                    $dest = array_slice($data_recv, 15, 4);
                    array_walk($dest, "hex2dec");
                    $type = $data_recv[19];
                    $seq1 = array_slice($data_recv, 25, 2);
                    array_walk($seq1, "hex2dec");
                    $source = join(".", $src);
        ...
        $this->callback("statistics", array(
            "hostname" => $this->hostname,
